...
def magnetorquer_out(mag_field, ang_vel, gain):
  ...
  best = [np.abs(dot(ang_vel,x)) for x in angular]
  best_index = np.argmax(best)
  # control_filter = 1
  # scaler_coef = control_filter/np.linalg.norm(mag_field)
  vec = scale(gain, angular[best_index]) 


  return best_index, angular[best_index], np.linalg.norm(angular[best_index]), vec
...
if __name__ == '__main__':
  ...
  torque_coil, mag, angular_ac, vec = magnetorquer_out(u, v, 50.0)
  ...
  v_new  = v.copy()
  v_new[torque_coil] = v[torque_coil] - change_in_rot


  print(torque_coil, mag)
  print(v_new)
